 ...
 *
 */
public class DefaultTcpNioSSLConnectionConfigurer implements TcpNioConnectionConfigurer, InitializingBean {

	private volatile SSLContext sslContext;

	private final TcpSSLContextConfigurer sslContextSupport;
	...
	public TcpNioConnection createNewConnection(SocketChannel socketChannel, boolean server, boolean lookupHost,
			ApplicationEventPublisher applicationEventPublisher, ConnectionFactorySupport connectionFactory) throws Exception {
		SSLEngine sslEngine = this.sslContext.createSSLEngine();
		TcpNioSSLConnection tcpNioSSLConnection = new TcpNioSSLConnection(socketChannel, server, lookupHost,
				applicationEventPublisher, connectionFactory, sslEngine);
